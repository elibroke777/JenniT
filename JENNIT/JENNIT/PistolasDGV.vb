...
Public Class PistolasDGV
    ...
    Private Sub PistolasDGV_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub AggPB_Click(sender As Object, e As EventArgs) Handles AggPB.Click
        Dim form As New FormPistolas() ' Se cambia la variable de la ventana FormPistolas

        If form.ShowDialog() = DialogResult.OK Then
            ' Para crear la Query y asi agg a la BD
            Dim cmd As New SqlCommand("INSERT INTO Pistolas (Marca, NoSerie, Etiqueta_HFI, Modelo) VALUES (@marca, @noserie, @etiqueta, @modelo)", conexion)
            ' Lineas que se le asignan a los valores en SQL
            ' Y que se agg desde el FormPistolas para Agg
    ...
    Private Sub ModificarPB_Click(sender As Object, e As EventArgs) Handles ModificarPB.Click
        ...
        Dim form As New FormPistolas() ' Actulizar el Registro
        form.EsModificar = True
        form.ID = Convert.ToInt32(row.Cells("id_Pistolas").Value)
        ...
        If form.ShowDialog() = DialogResult.OK Then
            ...
            Dim cmd As New SqlCommand("UPDATE Pistolas SET Marca=@marca, NoSerie=@noserie, Etiqueta_HFI=@etiqueta, Modelo=@modelo WHERE id_Pistolas=@id", conexion)
            cmd.Parameters.AddWithValue("@marca", form.Marca)
            cmd.Parameters.AddWithValue("@noserie", form.NoSerie)
            cmd.Parameters.AddWithValue("@etiqueta", form.Etiqueta_HFI)
            cmd.Parameters.AddWithValue("@modelo", form.Modelo)
    ...
    Private Sub EliminarPB_Click(sender As Object, e As EventArgs) Handles EliminarPB.Click
        ...
        Dim id As Integer = Convert.ToInt32(DataGridView1.SelectedRows(0).Cells("id_Pistolas").Value)
        ' Para ver cual ID seleccionaste
        Dim confirmacion = MessageBox.Show("¿Seguro que deseas eliminar esta pistola?", "Confirmar", MessageBoxButtons.YesNo)
        ' Para confirmar si vas a eliminar ese registro

        If confirmacion = DialogResult.Yes Then
            ' Query para eliminar el registro
            Dim cmd As New SqlCommand("DELETE FROM Pistolas WHERE id_Pistolas = @id", conexion)
    ...
    Private Sub ReturnPB_Click(sender As Object, e As EventArgs) Handles ReturnPB.Click
        Me.Close() ' Se cierra la ventana PISTOLASDGV
